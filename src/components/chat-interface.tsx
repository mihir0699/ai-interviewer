...
export function ChatInterface({
  ...
  const initialFetchInitiated = React.useRef(false);
    ...
    async (msgsForTranscript: Message[]) => {
      ...
      try {
        const previousAnswers = msgsForTranscript
          ...
          .map(
            (msg) =>
              `${msg.type === "ai" ? "Question" : "Answer"}: ${msg.content}`
          )
          .join("\n");
        ...
        setMessages((prev) => [
          ...prev,
          { id: Date.now().toString(), type: "ai", content: result.question },
        ]);
      } catch (error) {
        toast({
          title: "Error",
          description:
            error instanceof Error
              ? error.message
              : "Could not fetch the next question.",
          variant: "destructive",
        });
  ...
  React.useEffect(() => {
    ...
    // This ref ensures we only initiate the fetch for the initial question once.
        ...
        <div className="flex w-full items-start space-x-2">
          ...
          <div className="flex flex-col space-y-2">
            <Button
              ...
              disabled={
                isFetchingQuestion || isEndingInterview || !currentAnswer.trim()
              }
